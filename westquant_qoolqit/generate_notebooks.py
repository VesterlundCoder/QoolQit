...
combined_cells = [
    ...
    code(["# Installation (run once)\n",
          "%pip install -q \"qoolqit==1.4.0\"\n",
          "%pip install -q \"git+https://github.com/VesterlundCoder/QoolQit.git@qoolqit-contest-v1.1\""]),
    ...
    code(["from qoolqit import AnalogDeviceWithDMM\n",
          "\n",
          "mwis_weights = np.asarray(mwis_info['weights'])\n",
    ...
    md(["## 4. Combined Hierarchical Search — Factorial Experiment\n",
        ...
        "embedding candidates (3 families x 3 per family = 9), 3 replicates, 1000 shots.\n",
        "\n",
        "This matches the official `flagship_v3` configuration.\n",
        "\n",
        "**Metric definitions:**\n",
        "- **Conditional p_opt**: ground-state probability if a representation is physically feasible.\n",
        "- **End-to-end success** S = F * p_opt, where F = 1[terminal valid AND compilable].\n",
        "  Invalid/non-compilable representations receive S = 0.\n",
        "\n",
        "The end-to-end success metric gives a balanced factorial design for ANOVA:\n",
        "infeasible cells contribute 0 (not NaN), so no imputation is needed."]),
    code(["combined = combined_search(\n",
          "    h_problem, mwis_info=mwis_info,\n",
          "    n_hamiltonians=5, n_embedders=9, num_shots=1000, n_replicates=3, seed=42,\n",
          "    run_emulation=True, run_robustness=True, robustness_samples=3,\n",
          ")\n",
          "print(f'Factorial cells: {len(combined.cells)}')\n",
          "print(f'Hamiltonians: {len(combined.hamiltonian_ids)}')\n",
          "print(f'Embedders: {len(combined.embedder_ids)}')"]),
    md(["### Figure A — Physical Feasibility Across the H×R Representation Space\n",
        "\n",
        "Each cell is 1 (terminal-valid + compilable) or 0 (infeasible).\n",
        "This shows that representation choice affects whether the logical problem\n",
        "is successfully realized at all."]),
    code(["feasibility_mat = np.zeros((len(combined.hamiltonian_ids), len(combined.embedder_ids)))\n",
          "hidx = {h: i for i, h in enumerate(combined.hamiltonian_ids)}\n",
          "ridx = {r: i for i, r in enumerate(combined.embedder_ids)}\n",
          "for c in combined.cells:\n",
          "    if c.terminal_encoding_valid is True and c.compilation_success is True:\n",
          "        feasibility_mat[hidx[c.hamiltonian_id], ridx[c.embedder]] = 1.0\n",
          "\n",
          "fig, ax = plt.subplots(figsize=(10, 5))\n",
          "ax.imshow(feasibility_mat, cmap='RdYlGn', vmin=0, vmax=1, aspect='auto')\n",
          "ax.set_xticks(range(len(combined.embedder_ids)))\n",
          "ax.set_xticklabels(combined.embedder_ids, rotation=45, ha='right', fontsize=8)\n",
          "ax.set_yticks(range(len(combined.hamiltonian_ids)))\n",
          "ax.set_yticklabels(combined.hamiltonian_ids, fontsize=8)\n",
          "ax.set_xlabel('Embedding representation')\n",
          "ax.set_ylabel('Hamiltonian representation')\n",
          "ax.set_title('Physical Feasibility Across the H×R Representation Space')\n",
          "for i in range(feasibility_mat.shape[0]):\n",
          "    for j in range(feasibility_mat.shape[1]):\n",
          "        ax.text(j, i, '✓' if feasibility_mat[i, j] > 0 else '✗',\n",
          "                ha='center', va='center', fontsize=10,\n",
          "                color='white' if feasibility_mat[i, j] > 0 else 'red')\n",
          "plt.tight_layout(); plt.show()"]),
    md(["### Figure B — Ground-State Probability Conditional on Feasibility\n",
        "\n",
        "Only feasible cells show p_opt. Infeasible cells are visually marked\n",
        "as unavailable (gray), not zero."]),
    code(["popt_mat = combined.matrix('ground_state_probability')\n",
          "fig, ax = plt.subplots(figsize=(10, 5))\n",
          "masked = np.ma.masked_invalid(popt_mat)\n",
          "im = ax.imshow(masked, cmap='viridis', aspect='auto')\n",
          "# Overlay gray for infeasible cells\n",
          "infeasible_mask = np.isnan(popt_mat)\n",
          "for i in range(popt_mat.shape[0]):\n",
          "    for j in range(popt_mat.shape[1]):\n",
          "        if infeasible_mask[i, j]:\n",
          "            ax.add_patch(plt.Rectangle((j-0.5, i-0.5), 1, 1, facecolor='lightgray', edgecolor='gray'))\n",
          "        else:\n",
          "            ax.text(j, i, f'{popt_mat[i, j]:.3f}', ha='center', va='center',\n",
          "                    color='white' if popt_mat[i, j] < 0.5*popt_mat.max() else 'black', fontsize=8)\n",
          "ax.set_xticks(range(len(combined.embedder_ids)))\n",
          "ax.set_xticklabels(combined.embedder_ids, rotation=45, ha='right', fontsize=8)\n",
          "ax.set_yticks(range(len(combined.hamiltonian_ids)))\n",
          "ax.set_yticklabels(combined.hamiltonian_ids, fontsize=8)\n",
          "ax.set_xlabel('Embedding representation')\n",
          "ax.set_ylabel('Hamiltonian representation')\n",
          "ax.set_title('Ground-State Probability Conditional on Feasibility')\n",
          "fig.colorbar(im, ax=ax); plt.tight_layout(); plt.show()"]),
    md(["### Figure C — End-to-End Representation Success\n",
        "\n",
        "S = F * p_opt. This is the matrix from which the flagship ANOVA is derived.\n",
        "Infeasible cells contribute 0."]),
    code(["success_mat = combined.matrix('end_to_end_success')\n",
          "fig, ax = plt.subplots(figsize=(10, 5))\n",
          "im = ax.imshow(success_mat, cmap='viridis', aspect='auto')\n",
          "ax.set_xticks(range(len(combined.embedder_ids)))\n",
          "ax.set_xticklabels(combined.embedder_ids, rotation=45, ha='right', fontsize=8)\n",
          "ax.set_yticks(range(len(combined.hamiltonian_ids)))\n",
          "ax.set_yticklabels(combined.hamiltonian_ids, fontsize=8)\n",
          "ax.set_xlabel('Embedding representation')\n",
          "ax.set_ylabel('Hamiltonian representation')\n",
          "ax.set_title('End-to-End Representation Success (S = F × p_opt)')\n",
          "for i in range(success_mat.shape[0]):\n",
          "    for j in range(success_mat.shape[1]):\n",
          "        v = success_mat[i, j]\n",
          "        ax.text(j, i, f'{v:.3f}', ha='center', va='center', fontsize=8,\n",
          "                color='white' if v < 0.5*success_mat.max() else 'black')\n",
          "fig.colorbar(im, ax=ax); plt.tight_layout(); plt.show()"]),
    md(["### Variance Decomposition (End-to-End Success)\n",
        "\n",
        "Two-way ANOVA on the end-to-end success metric."]),
    code(["vd = combined.variance_decomposition('end_to_end_success')\n",
          "print(f'η²(H)   — Hamiltonian main effect:  {vd[\"eta2_H\"]:.1%}')\n",
    ...
    md(["## 5. Key Finding\n",
        ...
        "solutions that the preselected baseline cannot reach at all.\n",
